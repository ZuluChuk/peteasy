...
@Component({
    selector: 'jhi-order-entry-pe',
    templateUrl: './order-entry-pe.component.html'
})
export class OrderEntryPeComponent implements OnInit, OnDestroy {

currentAccount: any;
    orderEntries: OrderEntryPe[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/order-entry-pe'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/order-entry-pe', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: OrderEntryPe) {
        return item.id;
    }
